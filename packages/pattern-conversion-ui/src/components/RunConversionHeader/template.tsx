...
const Poject: FC<any> = () => {
  ...
  const processSorter = (sorters: any) =>{
      ...
      return sortParams.filter((s)=>s.order).map((s)=>`${s.filed},${s.order}`).join('|')
  }
...
// const loading = <div style={contentStyle} />;
  ...
  return <div className={styles.container}>
    ...
    <div className={styles.content_area_left}>
        ...
        <ProTable<DataType> columns={columns} dataSource={resources?.resources} loading={false} onChange={handleTableChange} onSubmit={handleTableSearch}/>
      </div>
